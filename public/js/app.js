...
async function createCountryFlow(){
  let availableFlags = [];
  try{
    const resFlags = await fetch('/flags/');
    availableFlags = await resFlags.json();
  }catch(e){
    availableFlags = ['flag_chern','flag_blue','flag_red'];
  }

  const name = await promptAsync("Введите название страны (буквы, цифры, пробелы, макс. 256)");
  if(!name || !name.match(/^[\p{L}\d\s]{1,256}$/u)) return alert("Некорректное название!");
...
async function givePointsFlow(){
  if(!(USER && USER.role === 'admin')) return alert('Нет прав');
  const id = await promptAsync("Введите ID страны");
