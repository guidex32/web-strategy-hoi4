...
async function logAction(user, action, details=''){
  try{
    await pool.query('INSERT INTO logs(user,action,details,timestamp) VALUES(?,?,?,NOW())',[user,action,details]);
...
app.post('/api/auth', async (req,res)=>{
  ...
  if(op==='register'){
    try{
      ...
      await logAction(login,'register');
...
app.post('/api', verifyToken, async (req,res)=>{
  const {op,countryId,unit,cost,attackerId,defenderId,name,x,y,amount,login,flag} = req.body;
  try{
    if(op==='toggle_economy'){
      ...
      await logAction(req.user.login,'toggle_economy',`now ${val?'ON':'OFF'}`);
      return res.json({ok:true,value:val});
    }

    if(op==='create_country' && req.user.role==='owner'){
      if(!name || /[0-9]/.test(name) || name.length>256) return res.json({ok:false,message:'Неправильное название'});
      if(!flag) return res.json({ok:false,message:'Флаг обязателен'});
      ...
      await logAction(req.user.login,'create_country',`name:${name},flag:${flag},x:${x},y:${y}`);
    ...
    if(op==='assign_owner' && req.user.role==='owner'){
      ...
      await logAction(req.user.login,'assign_owner',`countryId:${countryId} newOwner:${login}`);
      return res.json({ok:true});
    }
    ...
    if(op==='give_points'){
      ...
      await logAction(req.user.login,'give_points',`countryId:${countryId} amount:${amount}`);
      return res.json({ok:true});
    }

    // другие действия: buy_unit, declare_war, attack
    if(op==='buy_unit'){
      ...
      await logAction(req.user.login,'buy_unit',`countryId:${countryId} unit:${unit}`);
      return res.json({ok:true});
    }

    if(op==='declare_war'){
      await pool.query('UPDATE countries SET status=? WHERE id=?',['war',defenderId]);
      await logAction(req.user.login,'declare_war',`defenderId:${defenderId}`);
      return res.json({ok:true});
    }
    ...
    if(op==='attack'){
      ...
      await logAction(req.user.login,'attack',`defenderId:${defenderId} lost:10`);
...
// --- Начисление очков экономики каждый час ---
setInterval(async ()=>{
  try{
    const [rows] = await pool.query('SELECT value FROM settings WHERE name="economy"');
    const economy = rows.length ? rows[0].value==='1' : true;
    if(!economy) return;
    const [countries] = await pool.query('SELECT * FROM countries');
    for(const c of countries){
      const points = (c.points||0)+Math.floor(Math.random()*10)+1;
      await pool.query('UPDATE countries SET points=? WHERE id=?',[points,c.id]);
      await logAction('system','economy_add',`countryId:${c.id} +${points}`);
    }
