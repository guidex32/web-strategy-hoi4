...
app.post('/api/auth', async (req,res)=>{
  try {
    const { op, login, password } = req.body;
    if(!login || !password) return res.json({ok:false,message:'Логин и пароль обязательны'});
    ...
    if(op==='session'){
      ...
      if(!tokenHeader) return res.json({ok:false});
